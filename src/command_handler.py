...
@tree.command(name="add",
              ...
              guild=D_Object(id=credentials.GUILD_ID.value))
...
class SlashCommandListener:
  ...
  async def _shoutout(self, arg: str = ""):
    return await self.interaction.response.send_message(
      f"{self.interaction.user.name} says {arg}")
...
@tree.command(name="icpep",
              description="ICPEP Command List",
              guild=D_Object(id=credentials.GUILD_ID.value))
@app_commands.describe(execute="What kind of command to execute?",
                       arg="Additional Argument")
async def commands(interaction: D_Interaction,
                   execute: SLASHCOMMANDS,
                   arg: str = ""):
  slash_command = SlashCommandListener(interaction=interaction,
